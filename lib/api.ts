...
function authHeader(): Record<string, string> {
  const t = typeof window !== "undefined" ? localStorage.getItem("token") : undefined;
...
async function req<T>(path: string, init: RequestInit = {}): Promise<T> {
  ...
  if (!res.ok) {
    ...
    if (bodyText) {
      try { const j = JSON.parse(bodyText); msg = j?.message ?? bodyText; }
      catch { msg = bodyText; }
    }
    throw new Error(msg);
  }
...
export const api = {
  ...
  createTrade: (b: { asset: string; type: "buy" | "sell"; margin: number; leverage: number }) =>
  ...
  closeTrade: (orderId: string) =>
    req("/api/v1/trade/close", { method: "POST", body: JSON.stringify({ orderId }) }),
};
