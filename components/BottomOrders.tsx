      ...
      <div className="px-3 py-2 border-b border-neutral-800 flex gap-2 text-sm">
        {(["open","pending","closed"] as Tab[]).map(t => (
          <button key={t} onClick={()=>setTab(t)}
            className={`px-3 py-1.5 rounded-xl ${tab===t?"bg-blue-600 border-blue-700":"bg-neutral-800 border border-neutral-700"}`}>
        ...
        {tab==="open" && (
          <table className="w-full">
            ...
            <tbody>
              ...
              {!open.length && <tr><td className="py-6 text-center opacity-60" colSpan={5}>No open orders</td></tr>}
            </tbody>
          </table>
        )}

        {tab==="pending" && (
          <div className="opacity-60 text-sm">Pending orders UI TBD (not in backend spec).</div>
        )}
            ...
            <tbody>
              {closed.map(r => (
                <tr key={r.orderId} className="border-t border-neutral-800">
                  ...
                  <td className={`text-center ${r.pnl>=0?"text-emerald-400":"text-rose-400"}`}>
                    ${dollars(r.pnl)}
                  </td>
                </tr>
              ))}
              {!closed.length && <tr><td className="py-6 text-center opacity-60" colSpan={5}>No history yet</td></tr>}
            </tbody>
          </table>
        )}
      </div>
